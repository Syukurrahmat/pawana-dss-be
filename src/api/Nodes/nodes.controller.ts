...
export class NodesController {
    ...
    @Post('/')
    create(@Body() createDto: CreateNodeDto, @User() user: Users) {
        return this.service.create(createDto, user);
    }

    @Get('/')
    async findAll(@Query() pagination: PaginationQueryDto, @Query() filter: FindNodesDto) {
    ...
    @Get('/:id/datalogs')
    getDataLogs(@Param('id', ParseIntPipe) id: number, @Query() startEndDate: FindDatalogsDto) {
