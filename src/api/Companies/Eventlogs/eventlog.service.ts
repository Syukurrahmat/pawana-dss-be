...
export class EventlogsService {
    ...
    async findAll(companyId: number, monthQuery: string | undefined, tz: string) {
        const month =
            monthQuery && moment(monthQuery, 'YYYY-MM', true).isValid()
        ...
        const events = await this.getEventLogsInRange(companyId, startOfMonth, endOfMonth).then(
            (e) => Promise.all(e.map((f) => this.identifyEventStatus(f, tz)))
        );
    ...
    async getCurrentEventSummary(companyId: number, tz: string) {
        ...
        const [completed, inprogress, upcoming] = await Promise.all([
            this.EventLogsDB.findAndCountAll({
                ...
                where: {
                    companyId,
                    [Op.or]: [{ isCompleted: true }, { endDate: { [Op.lt]: startOfToday } }],
                },
                limit: 5,
                order: [['endDate', 'DESC']],
            }),
    ...
    async update(companyId: number, eventLogId: number, updateDto: UpdateEventDto) {
        ...
        if (startDate && endDate) {
            if (moment(endDate).isBefore(moment(startDate)))
                throw new BadRequestException('end date tidak boleh sebelum  start date ');
        }
    ...
    async getEvents(
        companyId: number,
        id: number,
        opt?: FindOptions<InferAttributes<EventLogs, { omit: never }>>
    ) {
    ...
    calculateEventDuration(
        ...
        tz: string,
        boundary?: { startDate: Moment; endDate: Moment }
    ) {
