...
export function categorizeCH4(value: number, recomendation?: boolean): GRKValue {
    const convertion = CH4ConversionTable.find((e) => e.limit >= value)
    return {
        value,
        category: convertion?.category,
        recomendation: convertion && recomendation
            ? convertion.recomendation
            : undefined
    };
}

export function categorizeCO2(value: number, recomendation?: boolean): GRKValue {
...
export function categorizeISPU(pm25Value: number, pm100Value: number, recomendation?: boolean): ISPUValue {
    ...
    const calculateForPolutant = (value: number, pollutant: "PM25" | "PM100"): ISPUValueItem => {
        const [ispuValue, floatISPUValue] = getIspuValue(value, pollutant);
        const convertion = value ? ISPUConvertionTable.find((e) => e.ISPU >= ispuValue) : undefined
